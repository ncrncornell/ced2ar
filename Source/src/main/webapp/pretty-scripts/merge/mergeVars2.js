...
$(document).ready(function (){
	baseHandle = $("#baseHandle").val();
	version = $("#version").val();
	varName = $("#varName").val();
	
	dmp = new diff_match_patch();
	showDiffs();	
	console.log("started diff");
});

//TODO: Flag var for later?
//TODO: Minor changes from variable version, might want to make modular
//Disable merging if no changes?

function showDiffs(){
	//TODO: How to handle if new fields were added to the local copy
...
function saveListen(){	
	$("#saveMerge").click(function(){
		...
		if(changedElements.length > 0){			
			...
			var path = "/ced2ar-web/edit/codebooks/"+baseHandle+"/v/"+version+"/vars/"+varName+"/merge";
			console.log(path);
