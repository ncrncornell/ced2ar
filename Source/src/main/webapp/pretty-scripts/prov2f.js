...
//TODO: Path(s) between nodes - isAuthor(), degrees of separation
...
var nodeColor = "rgba(42,100,150,0.9)";
var nodeColorSelected = "rgba(202,70,66,0.9)";
var nodeColorDiscovered = "rgba(25,60,90,0.9)";
var nodeColorDeselect = "rgba(42,100,150,0.25)";

var edgeColor = "rgba(200,200,200,.9)";
var edgeDeselectColor = "rgba(200,200,200,0.25)";
...
function buildGraph(){
	...
	drawGraph(startingNode,faConfig,1,true);
}
...
function toggleInfoBox(e){
	...
	if(showingInfo){
		...
		info += "<br /><button class='redrawLink btn' title='Set this node as the root' value='"+e.data.node.id+"'>";
		info += "<i class='fa fa-plus'></i> Redraw from here</button>";		
	
		if(!e.data.node.expanded){
			info += "<br /><button class='nodeExpand btn' title='Show more connected nodes' value='"+e.data.node.id+"'>";
			info += "<i class='fa fa-plus'></i> Expand from here</button>";		
		}else{
...
/**
 * Highlights incoming and outgoing nodes
 * @param id
 */
function recolor(id){	
	var selectedNodes = [];
