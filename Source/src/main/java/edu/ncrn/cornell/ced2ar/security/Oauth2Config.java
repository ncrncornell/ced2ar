...
import edu.ncrn.cornell.ced2ar.auth.oauth2.AuthFilter;
import edu.ncrn.cornell.ced2ar.auth.oauth2.AuthProvider;
import edu.ncrn.cornell.ced2ar.auth.oauth2.ClientContext;
...
public class Oauth2Config{
	...
	private String defaultFilterURL;
	private String infoReq;

    @Bean(name ="oAuth2ClientAuthenticationProcessingFilter" )
	public OAuth2ClientAuthenticationProcessingFilter oAuth2ClientAuthenticationProcessingFilter() {
    	populateOauth2Properties();
    	
    	if(StringUtils.isEmpty(defaultFilterURL))
    		defaultFilterURL = "/";
    	else
    		defaultFilterURL = "/"  +defaultFilterURL;
    	
    	OAuth2RestTemplate oAuth2RestTemplate =  new OAuth2RestTemplate(auth2ProtectedResourceDetails(), new ClientContext(accessTokenRequest));
    	AuthFilter authFilter = new AuthFilter(config,env,defaultFilterURL);
    ...
    private void populateOauth2Properties() {
    	if(config.isAuthenticationTypeOauth2()) {
			...
			this.accessTokenURL = env.getProperty("accessTokenURL");
			this.authURL = env.getProperty("authURL");
